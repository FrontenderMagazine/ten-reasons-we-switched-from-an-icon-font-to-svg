...
поделиться причинами, которые нас к этому привели, некоторыми 
ограничениями со стороны SVG, с которыми мы столкнулись, а также историей 
о том, как мы с ними справились.

## 1. Разделение функций

На [lonelyplanet.com][2] используется подключённый шрифт, и до недавнего времени 
иконки были упакованы в один файл вместе с ним, причём глифы хранились в области 
частного использования Unicode. Такое решение нас полностью устраивало, 
поскольку для загрузки всех иконок требовался только один HTTP-запрос, но, в 
то же время, оно ограничивало возможность управления приоритетом загрузки 
ресурсов.
...
после загрузки содержимого страницы, однако при предыдущем подходе к хранению 
...
Объединять подключённый шрифт и иконочный шрифт необязательно, их можно хранить 
отдельно. Наверное, мы бы так и сделали, если бы решили остановиться на 
...
казалась дилетантской и сломанной, что, без сомнения, послужило дополнительным 
...
что мы определили слепое пятно, которое оказались не в состоянии контролировать. 
...
SVG и PNG дают нам бо́льшую уверенность в том, что пользователь не увидит 
...
действует на нервы, но это некритично. А вот когда тот самый шрифт отвечает и 
за отображение иконок, страница внезапно заполняется мусором из чёрных квадратиков, 
в результате чего пользоваться ей становится невозможно.
...
точное их позиционирование нам ни разу не далось легко (как и одинаковое 
...
том же файле, его размер сжался до размера одной иконки (плюс около 50 байтов на 
...
Результаты использования обоих приёмов независимы от разрешения, масштабируемы и 
довольно легки по объёму, так что с точки зрения пользователя оба являются хорошими. 
